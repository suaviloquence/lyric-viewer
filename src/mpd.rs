use std::{collections::HashMap, net::TcpStream, io::{BufReader, BufRead, Write, Error, ErrorKind}};
...
pub fn connect(url: &str) -> std::io::Result<MPDClient> {
  ...
  if client.reader.read_line(&mut greeting)? == 0 || &greeting[..2] != "OK" {
		return Err(Error::new(ErrorKind::InvalidData, "Connection to MPD not OK."))
	}
